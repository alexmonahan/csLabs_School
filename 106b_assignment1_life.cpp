...
I created a "random" world, as seen in the code, and completed the extension for the wrap around. For my complete extension, please see
the other Life attached.*/
#include <cctype>
#include <cmath>
#include <fstream>
#include <iostream>
...
int main() {
    ...
    while(true){
        ...
        //if the user decides to begin the animation
            ...
            animate(grid, numFrames); //animate the grid with the given number of frames
        }
    ...
    for(int i = rowNum - 1; i <= rowNum + 1; i++){
        for(int j = colNum - 1; j <= colNum + 1; j++){
            if(!(i == rowNum && j == colNum)) {
                bool p = checkSpecificLocationForNeighbor(initGrid, i, j);
                if(p) numNeighbors++;
            }
        }
    }
